...
- **같은 `mode`끼리의 전환은 `end`/`start`를 건너뛴다.** 강의 모드가 말할 때마다 칠판을 다시 띄우고 인사하지
  않게 하는 규칙이다. 모드 전용 말하기 상태는 `talkState`(갈 곳)와 `talkReturn`(돌아올 곳)으로 이어진다.
  이 연결이 끊기면 `tools/widget_check.js`의 `lectureMode` 검사가 잡는다.
- **모드는 토글이라 `pinnedUntil = Infinity`로 잡고 `exitMode()`로만 푼다.** 켜 둔 동안 주사위가 돌면
  혼자 빠져나간다. `pickMotion`(다른 모션을 고를 때), 워치독 리셋, "계속 유지" 체크박스 셋 다 이 pin을
  건드리지 않도록 예외 처리가 들어가 있다 — 새 경로를 추가하면 같이 확인할 것.
...
- 머리 위 `Zzz`, `!`처럼 본체와 떨어진 기호는 매트 정리(`slice_and_key.py`의 최대 연결 성분 유지)에서 지워진다.
  표정과 동작으로 표현한다.
- **떨어진 큰 소품(칠판 등)은 프레임마다 생성하지 않는다.** 위 매트 정리에 지워지고, 매번 생성하면 모양이 흔들린다.
  한 번만 만들어 `sprites/props/`에 두고 키잉 뒤 `tools/compose_prop.py`로 뒤에 깔아 준다(`anims/*.json`의 `compose` 키).
  캐릭터의 좌우 이동도 거기서 처리하므로 생성 프레임은 늘 중앙 기준으로 뽑는다. 예: `lecture1_*`.
